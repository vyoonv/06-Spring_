...
memberPw.addEventListener("input", e => {
    ...
    if(inputPw.trim().length === 0) {
        ...
        checkObj.memberPw = false;  // 비밀번호 유효하지 않음 
        memberPw.value = ""; // 처음에 띄어쓰기 입력하지 않게 공백 제거 
        return; 
    }
...
// 8) 비밀번호 확인 유효성 검사  단, 비밀번호(memberPw)가 유효할 때만 검사 수행 
memberPwConfirm.addEventListener("input", () => {
    if(checkObj.memberPw) { // memberPw가 유효한 경우 (if문 수행이 true일때 이루어지니까)
        checkPw(); // 비교하는 함수 수행
        return; 
    }
...
memberNickname.addEventListener("input", e => {
    ...
    if( inputNickname.trim().length == 0) {
        ...
        nickMessage.innerText = "한글,영어,숫자로만 2~10글자 입력해주세요"; 
...
memberTel.addEventListener("input", e => {
    ...
    //const regExp = /^(02|0(3[1-3]|5[1-5]|6[1-4]|70|8[1-4]|9[0-4]))-?\d{3,4}-?\d{4}$|^(010|011|016|017|018|019)-?\d{3,4}-?\d{4}$/;
    const regExp = /^(02|0(3[1-3]|5[1-5]|6[1-4]|70|8[1-4]|9[0-4]))\d{3,4}\d{4}$|^(010|011|016|017|018|019)\d{3,4}\d{4}$/;


    // 유효하지 않은 경우 
    if(!regExp.test(inputMemberTel)) {
        telMessage.innerText = "알맞은 전화번호 형식으로 입력해주세요"; 
